...
class Admin::ContentController < Admin::BaseController
  ...
  def new_or_edit
    ...
    if params[:article]
      @article.attributes = params[:article].reject {|k,v| k == 'merge_with'}
    else
    ...
    if request.post?
      ...
      if params[:article] && params[:article].include?(:merge_with) then
        p params
        p @article
        @article.merge(params[:article][:merge_with])
      end
